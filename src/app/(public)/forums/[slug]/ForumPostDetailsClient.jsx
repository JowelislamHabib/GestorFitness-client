...
export default function ForumPostDetailsPage() {
  ...
  const RoleBadge = ({ role, className }) => {
    ...
    if (isTrainer) {
      return (
        <Badge className={`gap-1 bg-red-500/10 text-red-600 hover:bg-red-500/20 border-0 shadow-none uppercase tracking-wider text-[10px] ${className}`}>
            ...
            <Card className="p-4 sm:p-6 rounded-xl border border-border bg-zinc-100 dark:bg-zinc-900/50 flex gap-4 shadow-none">
              ...
              <form onSubmit={handleCommentSubmit} className="flex-1 flex flex-col gap-3">
                <Textarea 
                  ...
                  placeholder={session?.user?.isBlocked ? "Your account has been restricted by Admin." : (session?.user ? "Share your thoughts..." : "Please log in to join the discussion.")}
                  disabled={!session?.user || isSubmittingComment || session?.user?.isBlocked}
